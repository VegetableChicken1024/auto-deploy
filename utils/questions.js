...
const { createSSH, getSSHConfig, searchProjectFolders } = require('./ssh.js')
const { defaultConfig } = require('./config.js');
const createPrompt = (configKey, prompt) => {
  return defaultConfig?.[configKey] ? [] : [prompt];
};

const exitWithMessage = (message, delay = 3000) => {
  console.log(message);
  logger.info(message);
  setTimeout(() => {
    process.exit(0);
...
const question = [
  ...
  // 保存配置
  {
    ...
    message: '是否保存配置？',
    default: false,
    when: (answers) => answers.host && answers.username && answers.password && answers.port,
  },
  // 添加回滚询问
  {
    type: 'confirm',
  ...
  },
  {
    ...
    choices: async function (answers) {
      ...
      ssh.dispose();
      if (!stdout) {
        exitWithMessage('未搜索到历史版本，请检查项目路径是否正确，程序将在3秒后退出');
      }
      return stdout.split('\n').filter(file => file);
    },
    when: (answers) => answers.rollback,
...

  {
    ...
    choices: async function () {
      ...
      if (!zipFiles.length) {
        exitWithMessage('当前路径下未找到zip文件，请检查当前路径，程序将在3秒后退出');
      }
      return zipFiles;
    },
    when: (answers) => !answers.rollback,
...

  {
    ...
    default: function (answers) {
      ...
      const projectNameMatch = zipFileName.match(/^(.*?)(?:_v\d+\.\d+\.\d+_\d+)?\.zip$/);
